...
import diasil.math.geometry3.PointTransform;
...
public class TriangleMesh extends CoordinateSpace3 implements Intersectable
{
	...
	public TriangleMesh(Function3 f, SampleRange rx, SampleRange ry, PointTransform transform, Material m)
	{
		material = m;
		kdtree = new KDTree(3, 10);
		
		// wrapping currently not implemented
		...
		for (int i=0; i<rx.N; ++i)
		{
			...
			for (int j=0; j<ry.N; ++j)
			{
				...
				if (transform != null)
				{
					pts[k] = transform.transform(pts[k]);
				}
			}
		}
	...
	public Intersection getIntersection(Ray3 rw)
	{
		...
		return kdtree.getIntersection(ro);
	}
